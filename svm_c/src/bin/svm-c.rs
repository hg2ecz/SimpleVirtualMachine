...
use svm_c::common::model::{OptLevel, Target};
use svm_c::optimized::pipeline::compile_source;
...
fn usage() -> &'static str {
    "usage: svm-c --target register|stack|accumulator|memreg|loadstore|regmem|memory2memory|belt|tta [-O0|-O1|-O2|-Os] [-I dir|-Idir] [--emit asm|bin] source.sc [output]"
}
...
fn assembly_include_dirs(
    ...
    out.push(base.join("asm").join(tn));
    // A -I directory may itself be target-specific, or it may contain one
    // subdirectory per target. Prefer the target-specific child.
    for d in user {
        out.push(d.join(tn));
        out.push(d.clone());
    }
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            "--target" => {
                target = Some(match args.next().ok_or(usage())?.as_str() {
                    ...
                    _ => {
                        return Err("--target must be one of register, stack, accumulator, memreg, loadstore, regmem, memory2memory, belt, tta".into());
                    }
                });
            }
            "-I" => {
    ...
    let assembly =
        compile_source(&source, target, opt).map_err(|e| format!("compile error: {e}"))?;
