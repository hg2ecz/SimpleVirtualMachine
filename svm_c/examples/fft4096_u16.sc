...
// 4096-point radix-2 DIT FFT, Q15/u16 storage.
...
void stage_root(u16 len, u16 wr, u16 wi) {
    ...
    if (len == 512) { store16(wr, 0x7FFD); store16(wi, 0xFE6E); return; }
    if (len == 1024) { store16(wr, 0x7FFE); store16(wi, 0xFF37); return; }
    if (len == 2048) { store16(wr, 0x7FFF); store16(wi, 0xFF9B); return; }
    if (len == 4096) { store16(wr, 0x7FFF); store16(wi, 0xFFCE); return; }
...
void init_input() {
    ...
    for (i = 0; i < 4096; i++) {
        if (i < 2048) store16(re_addr(i), 0x7FFF);
