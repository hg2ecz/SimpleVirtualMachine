...
// 4096-point radix-2 DIT FFT using IEEE-754 binary16 bit patterns.
// f16 is stored in one u16 word; arithmetic is entirely software.
// real: 0x8000..., imag: 0xA000...

u16 re_addr(u16 i) { return 0x8000 + (i << 1); }
u16 im_addr(u16 i) { return 0xA000 + (i << 1); }
...
void stage_root(u16 len, u16 wr, u16 wi) {
    ...
    if (len == 512) { store16(wr, 0x3C00); store16(wi, 0xA248); return; }
    if (len == 1024) { store16(wr, 0x3C00); store16(wi, 0x9E48); return; }
    if (len == 2048) { store16(wr, 0x3C00); store16(wi, 0x9A48); return; }
    if (len == 4096) { store16(wr, 0x3C00); store16(wi, 0x9648); return; }
...
void init_input() {
    ...
    for (i = 0; i < 4096; i++) {
        if (i < 2048) store16(re_addr(i), 0x3C00);
