...
// 256-point radix-2 DIT FFT using software IEEE-754 binary32 arithmetic.
// Each f32 is a four-byte u32 object in explicit high RAM.
// real: 0x8000..., imag: 0x8400...

u16 re_addr(u16 i) { return 0x8000 + (i << 2); }
u16 im_addr(u16 i) { return 0x8400 + (i << 2); }
...
void init_input() {
    ...
    for (i = 0; i < 256; i++) {
        if (i < 128) f32_bits(re_addr(i), 0x0000, 0x3F80);
