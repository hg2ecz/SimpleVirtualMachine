//! Direct CLIR -> LoadStore lowering.
...
fn emit_operation(
    ...
    if matches!(op, "eq" | "ne" | "lt" | "le" | "gt" | "ge") {
        ...
        out.push_str(&format!("    {mnemonic} R0, R0, R1\n"));
        normalize(out, ty, "R0", label_id);
    }
...
fn parse_generated_u16(text: &str) -> Option<u16> {
    let text = text.trim();
...
fn legalize_logical_immediates(source: &str) -> Result<String, String> {
    let mut out = String::new();
    for line in source.lines() {
        let trimmed = line.trim();
        let translated = [("ANDI ", "AND"), ("ORI ", "OR"), ("XORI ", "XOR")]
            .into_iter()
            .find_map(|(pseudo, native)| trimmed.strip_prefix(pseudo).map(|rest| (native, rest)));
        if let Some((mnemonic, rest)) = translated {
            let (dst, imm) = rest
                .split_once(',')
                .ok_or_else(|| format!("bad logical immediate: {trimmed}"))?;
            let dst = dst.trim();
            let imm = imm.trim();
            if parse_generated_u16(imm).is_some_and(|value| value <= 63) {
                out.push_str(line);
                out.push('\n');
                continue;
            }
            if dst.eq_ignore_ascii_case("R5") {
                return Err("direct backend reserves R5 as scratch".into());
            }
            out.push_str(&format!("    MOVI R5, {imm}\n    {mnemonic} {dst}, R5\n"));
        } else {
            out.push_str(line);
            out.push('\n');
        }
...
pub fn lower(clir: &str) -> Result<String, String> {
    legalize_logical_immediates(&lower_raw(clir)?)
}
...
mod direct_tests {
    ...
    #[test]
    fn uses_native_three_operand_alu() {
        let c = "fn main() -> u16\n  const.u16 %0, 7\n  const.u16 %1, 5\n  add.u16 %2, %0, %1\n  ret %2\nend\n";
        let a = lower(c).unwrap();
        assert!(a.contains("ADD R0, R0, R1"));
    }

    #[test]
    fn keeps_small_native_logical_immediate() {
        let source = "    ANDI R0, 1\n";
        assert_eq!(legalize_logical_immediates(source).unwrap(), source);
    }
