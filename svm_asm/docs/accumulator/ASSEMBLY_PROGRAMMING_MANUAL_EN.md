...
`graphics.asm` exports the fast `gfx_set_color`, `gfx_set_palette`, `putpixel`, `clear`, `hline`, and `vline` primitives plus the higher-level `line`, `rect`, `fillrect`, `circle`, and `fillcircle` procedures. ABI: A=color; palette: X -> 4-byte table; putpixel: X=x, Y=y; clear: A=color; hline: X=x0, Y=x1, A=y; vline: A=x, X=y0, Y=y1. Unused procedures are removed by procedure-GC.
